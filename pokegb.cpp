...
#include                                                                                                      <SDL2/SDL.h>  /******************************************/
#include                                                                                                      "mman.h"  /*     POKEGB by Ben Smith+MINGC(2024.12.05)  */
#include                                                                                                        <unistd.h>  /* -------------------------------------- */
...
#define e(_,__)                                                              L(_,1)P __:f=(b-_)/8,d=b==__||!(F&aj[f])^f&1;  /******************************************/
    ...
    {E F=F&a|!b<<7|c<<6|d<<5|R<<4;}x J(x&b=k){a =i(b++      );E i(b++)<<8|a;}g t(x a){i(--w,a>>8,1);i(
   --w,a,1);E I+=4;}g q(g a,p b=1,g c=f){E(c&=7)==6?i      (n,a,b):b?*ae[c]=a:*ae[c];}g ah(p a,p b,p c){g*d=&      Z[a*16+b*2];E(d[1]>>c)%2*2+(*d>>c)%2;}p main(){T=(
  S=(g*)mmap(0,1<<20,1,1,open("PokemonRed.gb",0),0))+32768;c=open("rom.sav",258,0666);ftruncate(c,32768);SDL_setenv("SDL_AUDIODRIVER","directsound",1);SDL_Init(32+16+0);
  SDL_AudioDeviceID dev;SDL_AudioSpec wa, ha;wa.freq = 32768;wa.format = AUDIO_S16,wa.channels = 2;wa.samples = 546;wa.callback=au_c;wa.userdata = NULL;
  dev = SDL_OpenAudioDevice(NULL, 0, &wa, &ha, 0); au_i(); SDL_PauseAudioDevice(dev, 0); auto* aa = SDL_CreateRenderer(SDL_CreateWindow("2024PokeGBSound",20,30,800,720,4),-1,4);
 C=ad=(g*)mmap(0,32768,3,1,c,0);v=(g*)SDL_GetKeyboardState(0);z=145;af=44032;SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY,"1");auto*ab=SDL_CreateTexture(aa,376840196,1,160,144);
 for(;;){Y=I;if(A&H&h[511])H=V=A=0,I+=8,t(k),k=64;else if(V  )I+=4;else switch(b=i(k++)){M(1)*X[f]=J();P 0:K M(10)M  (2)c=b&8;q(i(*ai[f],j,!c),c,7);n+=f<2?0:5-2*f;K M(11)M
